...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        self.QACAttn = myModel.QuestionAwareContextLayer(
            myModel.AwareIntegration(myModel.AttentionScore(x1_d, x2_d, attention_hidden_size, opt), opt), opt)
        # default = 125
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask, overall_mask):
        ...
        def test(c, ques, tags):
            QAC = myModel.QuestionAwareContextLayer(contexts=c, questions=ques, tags=tags, opt=self.opt).forward()
            QAC = self.Wqac(QAC)
            return QAC
        ...
        doc_hiddens = self.Wqac(self.QACAttn(x1_input, x2_input, genTagsfromoverallmask(overall_mask)))
